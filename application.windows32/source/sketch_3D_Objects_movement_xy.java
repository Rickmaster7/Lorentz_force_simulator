...
public class sketch_3D_Objects_movement_xy extends PApplet {
...
int CL_carica = 0xff00FF00;
int CL_magneticField = 0xff006400;
...
public void switchMagneticField() {
  if (switchMagneticField == 2) {
    ...
    CL_magneticField = 0xffFFA500; //Orange 255,165,0
  } else {
    switchMagneticField = 2;
    CL_magneticField = 0xff006400;
  }
}
...
public void ON_OFF_carica() {
  if (ON_OFF_carica == 0) {
    ON_OFF_carica = 1;
    CL_carica = 0xffBA55D3;
  } else {
    ON_OFF_carica = 0;
    CL_carica = 0xff00FF00;
  }
}
...
public void setup() {
  ...
  MyController.getController("switchMagneticField").setColorBackground(0xff006400);
  ...
  MyController.getController("velocitySlider").setColorForeground(0xff4169E1);
  MyController.addSlider("forceSlider", 1, 100, 50, 50, 90, 200, 20);
  MyController.getController("forceSlider").setColorForeground(0xffDC143C);
  MyController.addSlider("phiSlider", -PI, PI, 0, 50, 150, 200, 20);
  MyController.getController("phiSlider").setColorForeground(0xff0F38FA);
...
public void protonExit() {
  ...
  if (phiSlider == 0 || phiSlider == -PI || phiSlider == PI) {
    ...
    sphere(sphereWeight);
    popMatrix();
    angleIncrease = bound(0.125f - r*0.001f); 
...
public void drawForce(float bottom, float top, float h, int sides, float rotation) {
  pushMatrix();
  rotateX(PI / 2);
  rotateY(rotation);
  rotateZ(PI / 2);
...
public void drawVelocity(float bottom, float top, float h, int sides, float rotation, int switchVelocityRotation) { //if switchVelocityRotation=1 clockwise
  pushMatrix();
  rotateX(PI / 2);
...
class MagneticField {
  ...
  public void drawMagneticField(float bottom, float top, float h, int sides, int rotation) {
    pushMatrix();
    rotateX(PI / 2);
    rotateY(PI / 2);
...
class Points {
  ...
  public void showPoints(ArrayList<Points> points, float speedX, float speedY) {
    for (Points p : points) {
      stroke(255);
      strokeWeight(2);
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "sketch_3D_Objects_movement_xy" };
    if (passedArgs != null) {
